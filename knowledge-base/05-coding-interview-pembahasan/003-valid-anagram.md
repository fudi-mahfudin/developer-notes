...
- **Topik utama:** String & HashMap
- **Inti masalah:** Memastikan dua string memiliki frekuensi karakter yang sama.

---

- Soal: Diberi dua string s dan t, tentukan apakah t adalah anagram dari s.
- Input: s: string, t: string
- Output: boolean
- Constraints utama: Karakter bisa berulang; perbedaan panjang langsung invalid.
- Pattern utama (Array / HashMap / Two Pointers / DP / Graph / dll): Frequency Count

## 2) Jawaban Ideal Singkat (30-60 detik)

> Saya mulai dari observasi bahwa pola soal ini paling cocok diselesaikan dengan **Frequency Count**. Strategi intinya adalah menyimpan informasi yang dibutuhkan agar keputusan di tiap langkah bisa diambil dalam waktu konstan atau linear. Dengan strategi ini, solusi tetap efisien untuk ukuran input besar dan tetap mudah dijelaskan saat interview. Saya juga cek edge case utama seperti input kosong, nilai duplikat, atau kombinasi khusus sesuai constraints. Hasil akhirnya memenuhi kebutuhan correctness dan performa.

Struktur cepat:
- Observasi inti masalah: Permasalahan bisa dipetakan ke pola Frequency Count.
- Strategi final yang dipilih: Gunakan Frequency Count untuk menghindari perhitungan berulang.
...
> Cek panjang dulu, lalu hitung frekuensi karakter dengan map/array count. O(n) time, O(1)/O(k) space.
...
decrement frequency using t
if any count is negative: false
return true
```
...
- Kasus normal: s='anagram', t='nagaram' -> true
- Kasus edge: s='a', t='ab' -> false
