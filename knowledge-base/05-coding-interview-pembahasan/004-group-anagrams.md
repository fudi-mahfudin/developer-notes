...
- **Inti masalah:** Mengelompokkan string yang punya komposisi huruf identik.

---

- Soal: Diberi array string, kelompokkan anagram dalam sub-array.
- Input: strs: string[]
- Output: string[][]
- Constraints utama: Jumlah string bisa besar; hindari perbandingan pairwise O(n^2).
- Pattern utama (Array / HashMap / Two Pointers / DP / Graph / dll): HashMap + Signature

## 2) Jawaban Ideal Singkat (30-60 detik)

> Saya mulai dari observasi bahwa pola soal ini paling cocok diselesaikan dengan **HashMap + Signature**. Strategi intinya adalah menyimpan informasi yang dibutuhkan agar keputusan di tiap langkah bisa diambil dalam waktu konstan atau linear. Dengan strategi ini, solusi tetap efisien untuk ukuran input besar dan tetap mudah dijelaskan saat interview. Saya juga cek edge case utama seperti input kosong, nilai duplikat, atau kombinasi khusus sesuai constraints. Hasil akhirnya memenuhi kebutuhan correctness dan performa.

Struktur cepat:
- Observasi inti masalah: Permasalahan bisa dipetakan ke pola HashMap + Signature.
- Strategi final yang dipilih: Gunakan HashMap + Signature untuk menghindari perhitungan berulang.
...
> Buat signature tiap kata (sorted string) lalu group di HashMap. O(n * k log k) time.
...
- Kasus normal: ['eat','tea','tan','ate','nat','bat'] -> [['eat','tea','ate'],['tan','nat'],['bat']]
- Kasus edge: [''] -> [['']]
