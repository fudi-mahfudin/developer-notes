# Bagaimana desain sistem admission pasien yang terintegrasi dengan WMS?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana desain sistem admission pasien yang terintegrasi dengan WMS?
...
> Desain admission pasien terintegrasi WMS saya mulai dari definisi event yang jelas: kapan admission memicu kebutuhan inventory, dan siapa owner data di tiap sistem. Saya hindari coupling ketat dengan memakai kontrak event yang stabil dan mekanisme retry idempotent. Sinkronisasi status saya buat eksplisit agar tim operasional tahu state mana yang authoritative saat terjadi delay. Pengalaman integrasi saya menunjukkan bahwa masalah utama biasanya bukan kirim data, tapi menjaga state tetap sinkron lintas boundary. Jadi desain admission saya berorientasi state consistency dan recoverability.

Struktur cepat:
- Konteks singkat: admission terintegrasi melibatkan state lintas sistem yang mudah drift.
- Aksi utama: tetapkan event contract, owner state, dan retry idempotent.
- Keputusan teknis/non-teknis: jaga sinkronisasi status lebih dulu sebelum optimasi lanjutan.
- Hasil terukur: konflik status lintas sistem lebih cepat diidentifikasi.
- Closing relevan ke posisi: saya terbiasa mendesain integrasi berbasis state yang tahan gangguan.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya desain integrasi admission-WMS berbasis event yang stabil dan state sinkron yang bisa dipulihkan saat terjadi delay.
...
- Pertanyaan: Bagaimana desain sistem admission pasien yang terintegrasi dengan WMS?
