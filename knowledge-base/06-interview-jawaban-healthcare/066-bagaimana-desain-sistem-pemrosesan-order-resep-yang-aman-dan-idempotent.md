# Bagaimana desain sistem pemrosesan order resep yang aman dan idempotent?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana desain sistem pemrosesan order resep yang aman dan idempotent?
...
> Untuk desain pemrosesan order resep yang aman dan idempotent, saya mulai dari failure mode paling berisiko: request ganda, stok berubah bersamaan, dan status order yang tidak sinkron. Saya desain endpoint dengan idempotency key, transaction boundary jelas, dan validasi stok yang terproteksi concurrency control. Setelah itu saya tambahkan audit trail agar setiap perubahan status order bisa ditelusuri. Alur ini saya pecah jadi step yang bisa dipulihkan jika satu tahap gagal, supaya tidak ada order setengah jadi. Fokus saya adalah patient safety dan konsistensi operasional, bukan cuma order berhasil tercatat.

Struktur cepat:
- Konteks singkat: order resep menyentuh data sensitif dan stok kritikal.
- Aksi utama: gabungkan idempotency, transaction control, dan auditability.
- Keputusan teknis/non-teknis: utamakan safety dan consistency dibanding throughput sesaat.
- Hasil terukur: duplicate dispense dan status order ambigu bisa ditekan.
- Closing relevan ke posisi: saya desain alur yang tahan gangguan dan bisa dipertanggungjawabkan.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya desain order resep dengan idempotency, kontrol konkurensi stok, dan audit trail agar aman untuk operasional healthcare.
...
- Pertanyaan: Bagaimana desain sistem pemrosesan order resep yang aman dan idempotent?
