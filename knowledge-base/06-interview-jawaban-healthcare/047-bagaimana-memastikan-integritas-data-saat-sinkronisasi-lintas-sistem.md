# Bagaimana memastikan integritas data saat sinkronisasi lintas sistem?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana memastikan integritas data saat sinkronisasi lintas sistem?
...
> Untuk menjaga integritas data saat sinkronisasi lintas sistem, saya selalu mulai dari source-of-truth yang tegas dan aturan konflik yang eksplisit. Setelah itu saya pasang validasi di setiap boundary sinkronisasi: sebelum kirim, sesudah terima, dan saat commit status final. Di integrasi healthcare, saya belajar bahwa asumsi “data pasti sama” adalah sumber masalah terbesar. Karena itu saya menyiapkan checksum/identifier transaksi yang konsisten agar mismatch bisa dideteksi cepat. Integritas data buat saya adalah disiplin proses dari ujung ke ujung, bukan hanya validasi satu endpoint.

Struktur cepat:
- Konteks singkat: sinkronisasi lintas sistem mudah rusak oleh asumsi data yang tidak sinkron.
- Aksi utama: tetapkan owner data dan validasi berlapis di setiap tahap sinkronisasi.
- Keputusan teknis/non-teknis: saya pilih proses ketat agar data tetap dapat dipercaya.
- Hasil terukur: mismatch lebih cepat terdeteksi dan dampak operasional berkurang.
- Closing relevan ke posisi: saya terbiasa membangun integritas data sebagai proses, bukan kebetulan.

## 3) Versi Ultra Singkat (10-20 detik)

> Integritas data saya jaga lewat source-of-truth jelas, validasi per boundary, dan deteksi mismatch yang cepat.
...
- Pertanyaan: Bagaimana memastikan integritas data saat sinkronisasi lintas sistem?
