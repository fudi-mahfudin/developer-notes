# Bagaimana cara menentukan status code HTTP yang tepat di kasus kompleks?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana cara menentukan status code HTTP yang tepat di kasus kompleks?
...
> Menentukan status code di kasus kompleks buat saya harus berbasis semantik error, bukan preferensi pribadi. Saya pakai 4xx untuk kesalahan dari sisi request/aturan bisnis klien, dan 5xx untuk kegagalan sistem internal atau dependency. Untuk kasus abu-abu, saya sepakatkan dulu decision rule di tim supaya konsisten lintas endpoint. Konsistensi ini penting karena status code jadi sinyal utama untuk monitoring dan retry logic di klien. Jadi saya tidak hanya pilih code yang benar, tapi juga yang paling operasional.

Struktur cepat:
- Konteks singkat: status code tidak konsisten merusak retry dan monitoring.
- Aksi utama: petakan rule status code berbasis jenis kegagalan.
- Keputusan teknis/non-teknis: konsistensi tim lebih penting dari selera individu.
- Hasil terukur: observability dan debugging jadi lebih presisi.
- Closing relevan ke posisi: ini memperkuat stabilitas integrasi klien-server.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya tentukan status code dari sumber masalahnya: client-side vs server-side, lalu jaga konsistensi lintas endpoint.
...
- Pertanyaan: Bagaimana cara menentukan status code HTTP yang tepat di kasus kompleks?
