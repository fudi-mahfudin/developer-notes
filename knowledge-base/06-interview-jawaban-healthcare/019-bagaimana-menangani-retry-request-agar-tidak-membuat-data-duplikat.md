# Bagaimana menangani retry request agar tidak membuat data duplikat?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana menangani retry request agar tidak membuat data duplikat?
...
> Untuk retry request agar tidak duplikat, saya gabungkan idempotency key, batas retry, dan observability pada jalur kritikal. Kuncinya bukan hanya menerima retry, tapi memastikan side effect dari request pertama tidak dieksekusi dua kali. Biasanya saya simpan status processing, result, dan timestamp agar request ulang punya perilaku deterministik. Di konteks healthcare, ini penting untuk mencegah order ganda atau perubahan stok yang tidak valid. Jadi retry saya desain aman sejak awal, bukan tambalan setelah incident.

Struktur cepat:
- Konteks singkat: retry tanpa guardrail bisa bikin data duplikat.
- Aksi utama: simpan state request berdasarkan idempotency key.
- Keputusan teknis/non-teknis: utamakan determinisme hasil pada request ulang.
- Hasil terukur: duplicate side effect bisa ditekan signifikan.
- Closing relevan ke posisi: reliability API meningkat tanpa menghambat user flow.

## 3) Versi Ultra Singkat (10-20 detik)

> Retry aman itu kombinasi idempotency key + status processing + batas retry, jadi side effect tidak pernah dieksekusi ganda.
...
- Pertanyaan: Bagaimana menangani retry request agar tidak membuat data duplikat?
