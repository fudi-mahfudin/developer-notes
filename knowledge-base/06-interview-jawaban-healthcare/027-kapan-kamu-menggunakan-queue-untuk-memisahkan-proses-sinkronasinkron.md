# Kapan kamu menggunakan queue untuk memisahkan proses sinkron/asinkron?

## 1) Pertanyaan

- Pertanyaan interviewer: Kapan kamu menggunakan queue untuk memisahkan proses sinkron/asinkron?
...
> Saya gunakan queue saat proses tidak wajib selesai dalam request-response user dan side effect-nya bisa diproses asinkron. Biasanya ini untuk workload berat, integrasi eksternal, atau proses yang perlu retry terkontrol. Dengan queue, latency endpoint utama turun dan sistem lebih tahan lonjakan traffic. Tapi saya tetap pasang idempotency dan monitoring worker supaya tidak pindah masalah ke belakang layar. Jadi keputusan pakai queue selalu saya ukur dari dampaknya ke reliability dan user experience.

Struktur cepat:
- Konteks singkat: proses sinkron berlebih bikin endpoint lambat dan rapuh.
- Aksi utama: pindahkan workload yang cocok ke jalur asynchronous.
- Keputusan teknis/non-teknis: saya jaga keseimbangan latency user dan konsistensi proses.
- Hasil terukur: respon API lebih cepat dan beban puncak lebih stabil.
- Closing relevan ke posisi: arsitektur jadi lebih resilien untuk trafik real.

## 3) Versi Ultra Singkat (10-20 detik)

> Queue saya pakai untuk proses berat/non-kritis di response user, dengan idempotency worker dan retry terkontrol.
...
- Pertanyaan: Kapan kamu menggunakan queue untuk memisahkan proses sinkron/asinkron?
