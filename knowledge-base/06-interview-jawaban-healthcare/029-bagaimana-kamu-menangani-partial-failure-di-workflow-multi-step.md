# Bagaimana kamu menangani partial failure di workflow multi-step?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana kamu menangani partial failure di workflow multi-step?
...
> Untuk partial failure di workflow multi-step, saya desain state yang eksplisit supaya tiap langkah punya status yang bisa diaudit dan di-recover. Saya hindari alur “setengah sukses tapi tidak terlihat” dengan menyiapkan kompensasi atau retry point yang jelas. Kalau step tertentu gagal, sistem tetap tahu harus rollback, retry, atau parkir untuk manual intervention. Pendekatan ini penting di domain healthcare karena satu kegagalan kecil bisa berdampak besar ke alur berikutnya. Intinya, saya desain workflow supaya failure bisa dikelola, bukan disembunyikan.

Struktur cepat:
- Konteks singkat: workflow multi-step rawan status menggantung saat satu langkah gagal.
- Aksi utama: definisikan state eksplisit dan jalur recovery untuk setiap langkah.
- Keputusan teknis/non-teknis: saya prioritaskan keterlacakan dan recoverability.
- Hasil terukur: kegagalan parsial lebih cepat dipulihkan dan lebih mudah diaudit.
- Closing relevan ke posisi: sistem tetap terkontrol walau dependency tidak selalu sehat.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya tangani partial failure dengan state machine jelas + kompensasi/retry point, jadi kegagalan tidak jadi black box.
...
- Pertanyaan: Bagaimana kamu menangani partial failure di workflow multi-step?
