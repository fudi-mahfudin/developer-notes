# Kapan harus gunakan Promise.all vs Promise.allSettled?

## 1) Pertanyaan

- Pertanyaan interviewer: Kapan harus gunakan Promise.all vs Promise.allSettled?
...
> Saya pakai `Promise.all` saat semua task wajib sukses untuk melanjutkan proses, misalnya komposisi response yang setiap komponennya mandatory. Kalau saya perlu melihat hasil semua task meski ada yang gagal, saya pilih `Promise.allSettled` untuk dapat gambaran lengkap dan melakukan handling parsial. Strategi saya adalah dependency-sensitive concurrency, yaitu pola concurrency ditentukan oleh ketergantungan bisnis antar task. Ini membantu menghindari kegagalan berantai yang tidak perlu. Jadi pemilihannya bukan preferensi sintaks, tapi keputusan terhadap model failure yang diinginkan.

Struktur cepat:
- Konteks singkat: pola concurrency memengaruhi ketahanan workflow saat ada failure.
- Aksi utama: tentukan dulu apakah hasil parsial bisa diterima bisnis.
- Keputusan teknis/non-teknis: gunakan `all` untuk hard dependency, `allSettled` untuk graceful handling.
- Hasil terukur: error handling lebih tepat dan alur bisnis lebih resilient.
- Closing relevan ke posisi: saya memetakan kebutuhan bisnis sebelum menentukan primitive concurrency.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya memilih `all` atau `allSettled` dengan strategi dependency-sensitive concurrency berdasarkan apakah kegagalan satu task harus menggagalkan seluruh flow.
...
- Definisi strategi: dependency-sensitive concurrency = model paralelisme disesuaikan dengan aturan ketergantungan hasil dan toleransi kegagalan proses bisnis.
...
- Pertanyaan: Kapan harus gunakan Promise.all vs Promise.allSettled?
