# Bagaimana menyusun alert agar tidak noisy?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana menyusun alert agar tidak noisy?
...
> Alert saya susun agar tidak noisy dengan prinsip actionable-only: hanya memicu jika butuh aksi manusia sekarang. Strategi saya signal-to-action alerting: threshold disetel dari baseline dan error budget, bukan angka default. Saya juga pakai dedup/suppression untuk mencegah banjir notifikasi saat insiden besar.

Struktur cepat:
- Konteks singkat: pada kasus "Bagaimana menyusun alert agar tidak noisy?", saya identifikasi risiko operasional paling kritikal lebih dulu.
...
> Saya desain alert yang actionable dengan threshold berbasis baseline supaya tidak noisy tapi tetap sensitif.
...
- Definisi strategi: signal-to-action alerting = merancang alert yang seimbang antara sensitivitas dan kegunaan tindakan.
...
- Pertanyaan: Bagaimana menyusun alert agar tidak noisy?
