# Cara mengamankan API dari SQL injection?

## 1) Pertanyaan

- Pertanyaan interviewer: Cara mengamankan API dari SQL injection?
...
> Mengamankan API dari SQL injection saya lakukan dengan prepared statement/ORM parameterized query sebagai baseline, bukan string concatenation. Strategi saya query boundary hardening: semua input user dianggap tidak terpercaya hingga tervalidasi dan diparameterisasi. Saya tambah validasi schema dan least-privilege DB account agar dampak jika ada bypass tetap terbatas.

Struktur cepat:
- Konteks singkat: SQL injection bisa langsung berdampak ke kerahasiaan dan integritas data.
- Aksi utama: enforce parameterized query di seluruh jalur akses DB.
- Keputusan teknis/non-teknis: gabungkan schema validation dengan privilege separation.
- Hasil terukur: attack surface query dinamis turun drastis.
- Closing relevan ke posisi: saya menutup risiko dari desain query sampai konfigurasi database.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya cegah SQL injection dengan query boundary hardening: parameterized query, validasi input, dan privilege DB minimum.
...
- Definisi strategi: query boundary hardening = memperketat batas antara input eksternal dan eksekusi query database.
...
- Pertanyaan: Cara mengamankan API dari SQL injection?
