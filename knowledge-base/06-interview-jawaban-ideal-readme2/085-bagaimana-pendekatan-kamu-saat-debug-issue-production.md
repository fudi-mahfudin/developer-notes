# Bagaimana pendekatan kamu saat debug issue production?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana pendekatan kamu saat debug issue production?
...
> Saat debug issue production, saya mulai dari dampak user dan metrik utama dulu sebelum masuk ke detail implementasi. Strategi saya symptom-to-root debugging: observasi gejala, bentuk hipotesis, validasi cepat, lalu konfirmasi root cause. Pendekatan terstruktur ini menghindari trial-and-error yang menghabiskan waktu incident.

Struktur cepat:
- Konteks singkat: pada kasus "Bagaimana pendekatan kamu saat debug issue production?", saya mulai dari risiko tertinggi yang memengaruhi user dan operasi.
...
> Saya debug production dengan symptom-to-root: mulai dari sinyal dampak, lalu validasi hipotesis sampai akar masalah.
...
- Definisi strategi: symptom-to-root debugging = metodologi debugging dari gejala terukur menuju akar masalah terverifikasi.
...
- Pertanyaan: Bagaimana pendekatan kamu saat debug issue production?
