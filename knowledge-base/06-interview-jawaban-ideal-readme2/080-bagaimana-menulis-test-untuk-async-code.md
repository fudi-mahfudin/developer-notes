# Bagaimana menulis test untuk async code?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana menulis test untuk async code?
...
> Untuk async code, saya selalu menguji success path, timeout path, retry path, dan error propagation secara eksplisit. Strategi saya async-failure-oriented testing: fokus test bukan hanya hasil berhasil, tapi juga perilaku saat kegagalan. Ini penting di Node.js karena bug async sering tersembunyi sampai produksi jika hanya diuji happy path.

Struktur cepat:
- Konteks singkat: pada kasus "Bagaimana menulis test untuk async code?", saya mulai dari risiko tertinggi yang memengaruhi user dan operasi.
...
> Saya uji async code dengan fokus failure path: timeout, retry, dan error propagation, bukan hanya happy path.
...
- Definisi strategi: async-failure-oriented testing = merancang test async dengan prioritas pada skenario gagal yang paling berisiko.
...
- Pertanyaan: Bagaimana menulis test untuk async code?
