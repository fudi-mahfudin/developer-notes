...
- Inti masalah (1-2 kalimat): Menyelesaikan validate binary search tree dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Validate Binary Search Tree.
...
function validateBinarySearchTree(root) {
  if (!root) return 0;
  return 1 + Math.max(validateBinarySearchTree(root.left), validateBinarySearchTree(root.right));
}
```
