...
- Inti masalah (1-2 kalimat): Menyelesaikan two sum dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Two Sum.
...
- Pola yang terdeteksi: Array/HashMap.
...
- Ide: Terapkan pendekatan Array/HashMap yang paling lazim dipakai untuk problem ini.
...
function twoSum(nums, target) {
  ...
  for (let i = 0; i < nums.length; i += 1) {
    ...
    if (indexMap.has(need)) return [indexMap.get(need), i];
