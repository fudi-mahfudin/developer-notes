...
- Inti masalah (1-2 kalimat): Menyelesaikan remove nth node from end dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Remove Nth Node From End.
...
- Pola yang terdeteksi: Linked List/Two Pointers.
...
- Ide: Terapkan pendekatan Linked List/Two Pointers yang paling lazim dipakai untuk problem ini.
...
function removeNthNodeFromEndOfList(head) {
  let prev = null;
  let curr = head;
  while (curr) {
