...
- Topik utama: Tree
- Inti masalah (1-2 kalimat): Menyelesaikan subtree of another tree dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Subtree of Another Tree.
...
- Ide: Terapkan pendekatan Tree yang paling lazim dipakai untuk problem ini.
...
function subtreeOfAnotherTree(root) {
  if (!root) return 0;
  return 1 + Math.max(subtreeOfAnotherTree(root.left), subtreeOfAnotherTree(root.right));
}
```
