...
- Inti masalah (1-2 kalimat): Menyelesaikan palindrome partitioning dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Palindrome Partitioning.
...
function palindromePartitioning(nums) {
  const result = [];
  const path = [];
  function dfs(index) {
