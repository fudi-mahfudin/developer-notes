...
- Topik utama: Binary Search/Design
- Inti masalah (1-2 kalimat): Menyelesaikan time based key-value store dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Time Based Key-Value Store.
...
- Pola yang terdeteksi: Binary Search/Design.
...
- Ide: Terapkan pendekatan Binary Search/Design yang paling lazim dipakai untuk problem ini.
...
function timeBasedKeyValueStore(nums, target) {
  let left = 0;
