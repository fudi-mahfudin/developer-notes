...
- Inti masalah (1-2 kalimat): Menyelesaikan letter combinations of a phone number dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Letter Combinations of a Phone Number.
...
- Pola yang terdeteksi: Backtracking.
...
- Ide: Terapkan pendekatan Backtracking yang paling lazim dipakai untuk problem ini.
...
function letterCombinationsOfAPhoneNumber(nums) {
  const result = [];
  const path = [];
  function dfs(index) {
