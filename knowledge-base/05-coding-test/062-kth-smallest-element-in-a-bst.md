...
- Topik utama: Tree/Inorder
- Inti masalah (1-2 kalimat): Menyelesaikan kth smallest element in bst dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Kth Smallest Element in BST.
...
- Pola yang terdeteksi: Tree/Inorder.
...
- Ide: Terapkan pendekatan Tree/Inorder yang paling lazim dipakai untuk problem ini.
...
function kthSmallestElementInABst(root) {
  if (!root) return 0;
  return 1 + Math.max(kthSmallestElementInABst(root.left), kthSmallestElementInABst(root.right));
}
```
