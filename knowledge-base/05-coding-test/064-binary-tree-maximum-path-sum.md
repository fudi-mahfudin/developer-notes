...
- Inti masalah (1-2 kalimat): Menyelesaikan binary tree maximum path sum dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Binary Tree Maximum Path Sum.
...
function binaryTreeMaximumPathSum(root) {
  if (!root) return 0;
  return 1 + Math.max(binaryTreeMaximumPathSum(root.left), binaryTreeMaximumPathSum(root.right));
}
```
