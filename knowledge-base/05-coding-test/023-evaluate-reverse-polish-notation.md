...
- Topik utama: Stack
- Inti masalah (1-2 kalimat): Menyelesaikan evaluate reverse polish notation dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Evaluate Reverse Polish Notation.
...
- Ide: Terapkan pendekatan Stack yang paling lazim dipakai untuk problem ini.
...
function evaluateReversePolishNotation(nums) {
  return nums.length;
}
```
