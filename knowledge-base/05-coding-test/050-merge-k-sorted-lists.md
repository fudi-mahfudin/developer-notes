...
- Topik utama: Linked List/Heap
- Inti masalah (1-2 kalimat): Menyelesaikan merge k sorted lists dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Merge K Sorted Lists.
...
- Pola yang terdeteksi: Linked List/Heap.
...
- Ide: Terapkan pendekatan Linked List/Heap yang paling lazim dipakai untuk problem ini.
...
function mergeKSortedLists(head) {
  let prev = null;
  let curr = head;
  while (curr) {
