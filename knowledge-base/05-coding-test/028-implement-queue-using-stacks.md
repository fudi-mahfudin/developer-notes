...
- Topik utama: Queue/Stack
- Inti masalah (1-2 kalimat): Menyelesaikan implement queue using stacks dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Implement Queue using Stacks.
...
- Pola yang terdeteksi: Queue/Stack.
...
- Ide: Terapkan pendekatan Queue/Stack yang paling lazim dipakai untuk problem ini.
...
function implementQueueUsingStacks(nums) {
  return nums.length;
}
```
