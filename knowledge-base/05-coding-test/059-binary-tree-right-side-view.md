...
- Topik utama: Tree/BFS
- Inti masalah (1-2 kalimat): Menyelesaikan binary tree right side view dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Binary Tree Right Side View.
...
- Ide: Terapkan pendekatan Tree/BFS yang paling lazim dipakai untuk problem ini.
...
function binaryTreeRightSideView(root) {
  if (!root) return 0;
  return 1 + Math.max(binaryTreeRightSideView(root.left), binaryTreeRightSideView(root.right));
}
```
