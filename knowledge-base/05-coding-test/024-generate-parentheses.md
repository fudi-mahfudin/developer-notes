...
- Topik utama: Backtracking/Stack
- Inti masalah (1-2 kalimat): Menyelesaikan generate parentheses dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Generate Parentheses.
...
- Pola yang terdeteksi: Backtracking/Stack.
...
- Ide: Terapkan pendekatan Backtracking/Stack yang paling lazim dipakai untuk problem ini.
...
function generateParentheses(nums) {
  const result = [];
  const path = [];
  function dfs(index) {
