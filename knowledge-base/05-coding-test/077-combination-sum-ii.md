...
- Inti masalah (1-2 kalimat): Menyelesaikan combination sum ii dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Combination Sum II.
...
function combinationSumIi(nums) {
  const result = [];
  const path = [];
  function dfs(index) {
