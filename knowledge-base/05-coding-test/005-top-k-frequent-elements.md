...
- Topik utama: Array/HashMap/Heap
- Inti masalah (1-2 kalimat): Menyelesaikan top k frequent elements dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Top K Frequent Elements.
...
- Pola yang terdeteksi: Array/HashMap/Heap.
...
- Ide: Terapkan pendekatan Array/HashMap/Heap yang paling lazim dipakai untuk problem ini.
...
function topKFrequentElements(nums) {
  return nums.length;
}
```
