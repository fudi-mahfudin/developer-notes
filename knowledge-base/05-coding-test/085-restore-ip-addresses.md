...
- Inti masalah (1-2 kalimat): Menyelesaikan restore ip addresses dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Restore IP Addresses.
...
function restoreIpAddresses(nums) {
  const result = [];
  const path = [];
  function dfs(index) {
