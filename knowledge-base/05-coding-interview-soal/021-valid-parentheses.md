...
- Topik utama: Stack
- Inti masalah (1-2 kalimat): Menyelesaikan valid parentheses dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Valid Parentheses.
...
function isValid(s) {
  ...
  for (const ch of s) {
    if (pair.has(ch)) {
      if (stack.pop() !== pair.get(ch)) return false;
    } else {
      stack.push(ch);
    }
  }
