...
- Inti masalah (1-2 kalimat): Menyelesaikan two sum ii (sorted) dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Two Sum II (Sorted).
...
- Pola yang terdeteksi: Two Pointers.
...
- Ide: Terapkan pendekatan Two Pointers yang paling lazim dipakai untuk problem ini.
...
function twoSumIiInputArrayIsSorted(nums) {
  return nums.length;
}
```
