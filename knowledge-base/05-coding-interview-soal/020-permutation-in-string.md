...
- Inti masalah (1-2 kalimat): Menyelesaikan permutation in string dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Permutation in String.
...
function permutationInString(nums) {
  return nums.length;
}
```
