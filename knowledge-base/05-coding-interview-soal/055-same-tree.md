...
- Inti masalah (1-2 kalimat): Menyelesaikan same tree dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Same Tree.
...
function sameTree(root) {
  if (!root) return 0;
  return 1 + Math.max(sameTree(root.left), sameTree(root.right));
}
```
