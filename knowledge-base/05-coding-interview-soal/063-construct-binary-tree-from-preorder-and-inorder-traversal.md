...
- Topik utama: Tree/Recursion
- Inti masalah (1-2 kalimat): Menyelesaikan construct tree from traversals dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Construct Tree from Traversals.
- Output: Hasil akhir sesuai format yang diminta soal.
- Constraints: Jumlah node umumnya hingga 10^5; traversal harus linear terhadap node+edge.
- Contoh input-output:
...
- Pola yang terdeteksi: Tree/Recursion.
...
- Ide: Terapkan pendekatan Tree/Recursion yang paling lazim dipakai untuk problem ini.
...
function constructBinaryTreeFromPreorderAndInorderTraversal(root) {
  if (!root) return 0;
  return 1 + Math.max(constructBinaryTreeFromPreorderAndInorderTraversal(root.left), constructBinaryTreeFromPreorderAndInorderTraversal(root.right));
}
```
