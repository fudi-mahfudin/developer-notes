...
- Inti masalah (1-2 kalimat): Menyelesaikan longest repeating character replacement dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Longest Repeating Character Replacement.
...
- Pola yang terdeteksi: Sliding Window.
...
- Ide: Terapkan pendekatan Sliding Window yang paling lazim dipakai untuk problem ini.
...
function longestRepeatingCharacterReplacement(nums) {
  return nums.length;
}
```
