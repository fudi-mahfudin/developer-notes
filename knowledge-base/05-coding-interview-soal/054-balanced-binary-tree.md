...
- Inti masalah (1-2 kalimat): Menyelesaikan balanced binary tree dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Balanced Binary Tree.
...
function balancedBinaryTree(root) {
  if (!root) return 0;
  return 1 + Math.max(balancedBinaryTree(root.left), balancedBinaryTree(root.right));
}
```
