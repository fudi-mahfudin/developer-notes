...
- Inti masalah (1-2 kalimat): Menyelesaikan capacity to ship packages within d days dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Capacity To Ship Packages Within D Days.
- Output: Hasil akhir sesuai format yang diminta soal.
- Constraints: 1 <= n <= 10^5, nilai elemen di rentang -10^9 hingga 10^9 (sesuaikan varian soal).
- Contoh input-output:
...
- Pola yang terdeteksi: Binary Search on Answer.
...
- Ide: Terapkan pendekatan Binary Search on Answer yang paling lazim dipakai untuk problem ini.
...
function capacityToShipPackagesWithinDDays(nums, target) {
  let left = 0;
