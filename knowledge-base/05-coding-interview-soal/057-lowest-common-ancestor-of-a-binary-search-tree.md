...
- Topik utama: Tree/BST
- Inti masalah (1-2 kalimat): Menyelesaikan lca of bst dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal LCA of BST.
...
- Ide: Terapkan pendekatan Tree/BST yang paling lazim dipakai untuk problem ini.
...
function lowestCommonAncestorOfABinarySearchTree(root) {
  if (!root) return 0;
  return 1 + Math.max(lowestCommonAncestorOfABinarySearchTree(root.left), lowestCommonAncestorOfABinarySearchTree(root.right));
}
```
