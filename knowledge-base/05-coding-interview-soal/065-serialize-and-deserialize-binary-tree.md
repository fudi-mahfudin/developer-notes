...
- Topik utama: Tree/Design
- Inti masalah (1-2 kalimat): Menyelesaikan serialize and deserialize binary tree dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Serialize and Deserialize Binary Tree.
...
- Pola yang terdeteksi: Tree/Design.
...
- Ide: Terapkan pendekatan Tree/Design yang paling lazim dipakai untuk problem ini.
...
function serializeAndDeserializeBinaryTree(root) {
  if (!root) return 0;
  return 1 + Math.max(serializeAndDeserializeBinaryTree(root.left), serializeAndDeserializeBinaryTree(root.right));
}
```
