...
- Topik utama: DP
- Inti masalah (1-2 kalimat): Menyelesaikan partition equal subset sum dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Partition Equal Subset Sum.
...
- Ide: Terapkan pendekatan DP yang paling lazim dipakai untuk problem ini.
...
function partitionEqualSubsetSum(n) {
  if (n <= 1) return n;
