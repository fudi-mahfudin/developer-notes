...
- Inti masalah (1-2 kalimat): Menyelesaikan merge two sorted lists dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Merge Two Sorted Lists.
...
- Pola yang terdeteksi: Linked List.
...
- Ide: Terapkan pendekatan Linked List yang paling lazim dipakai untuk problem ini.
...
function mergeTwoSortedLists(head) {
  let prev = null;
  let curr = head;
  while (curr) {
