...
- Inti masalah (1-2 kalimat): Menyelesaikan invert binary tree dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Invert Binary Tree.
...
function invertBinaryTree(root) {
  if (!root) return 0;
  return 1 + Math.max(invertBinaryTree(root.left), invertBinaryTree(root.right));
}
```
