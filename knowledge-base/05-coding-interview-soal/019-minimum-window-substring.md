...
- Inti masalah (1-2 kalimat): Menyelesaikan minimum window substring dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Minimum Window Substring.
...
function minimumWindowSubstring(nums) {
  return nums.length;
}
```
