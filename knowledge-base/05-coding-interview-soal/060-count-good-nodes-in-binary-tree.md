...
- Inti masalah (1-2 kalimat): Menyelesaikan count good nodes in binary tree dengan strategi yang benar secara logika, efisien terhadap constraints, dan aman terhadap edge case klasik interview.

## 2) Detail Soal

- Input: Parameter mengikuti definisi soal Count Good Nodes in Binary Tree.
...
- Ide: Terapkan pendekatan Tree/DFS yang paling lazim dipakai untuk problem ini.
...
function countGoodNodesInBinaryTree(root) {
  if (!root) return 0;
  return 1 + Math.max(countGoodNodesInBinaryTree(root.left), countGoodNodesInBinaryTree(root.right));
}
```
