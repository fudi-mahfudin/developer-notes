# Bagaimana desain audit log immutable untuk data medis sensitif?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana desain audit log immutable untuk data medis sensitif?
...
> Audit log immutable untuk data medis sensitif saya desain sebagai append-only event store yang tidak bisa diubah lewat jalur aplikasi normal. Setiap event mencatat actor, alasan perubahan, before/after ringkas, dan timestamp yang sinkron. Saya pisahkan akses baca audit dari akses operasi harian untuk mengurangi risiko manipulasi. Selain itu, saya pasang retention policy dan kontrol akses ketat karena audit data medis punya konsekuensi kepatuhan. Bagi saya, immutable audit log itu fondasi trust, bukan fitur tambahan.

Struktur cepat:
- Konteks singkat: data medis sensitif butuh jejak perubahan yang benar-benar dapat dipercaya.
- Aksi utama: implementasikan append-only audit event dengan metadata lengkap.
- Keputusan teknis/non-teknis: isolasi akses audit dan perketat permission model.
- Hasil terukur: investigasi dan kepatuhan audit jadi lebih kuat.
- Closing relevan ke posisi: saya menempatkan trust dan compliance sebagai prioritas desain.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya bangun audit log medis sebagai append-only dengan akses ketat agar jejak perubahan tidak bisa dimanipulasi.
...
- Pertanyaan: Bagaimana desain audit log immutable untuk data medis sensitif?
