# Bagaimana desain sinkronisasi stok farmasi lintas cabang rumah sakit?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana desain sinkronisasi stok farmasi lintas cabang rumah sakit?
...
> Sinkronisasi stok farmasi lintas cabang saya desain dengan model sumber data utama yang jelas dan replikasi perubahan yang terkontrol. Saya pakai event per perubahan stok dengan idempotency guard supaya replay tidak mengubah jumlah dua kali. Untuk konsistensi antar cabang, saya tambah proses reconciliation berkala dan alarm jika selisih melewati ambang. Saya juga bedakan alur real-time kritikal dengan alur batch yang bisa ditunda. Dengan desain ini, sistem tetap scalable tanpa kehilangan kontrol akurasi stok.

Struktur cepat:
- Konteks singkat: stok lintas cabang rawan drift karena delay dan replay event.
- Aksi utama: gunakan event perubahan stok dengan dedup/idempotency.
- Keputusan teknis/non-teknis: kombinasikan sinkronisasi near real-time dan rekonsiliasi periodik.
- Hasil terukur: selisih stok antar cabang lebih cepat terdeteksi dan ditutup.
- Closing relevan ke posisi: saya menyeimbangkan scale dan akurasi operasional.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya sinkronkan stok lintas cabang lewat event idempotent + reconciliation berkala agar akurat dan tetap scalable.
...
- Pertanyaan: Bagaimana desain sinkronisasi stok farmasi lintas cabang rumah sakit?
