# Bagaimana kamu menangani dependency eksternal yang sering timeout?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana kamu menangani dependency eksternal yang sering timeout?
...
> Saat dependency eksternal sering timeout, saya hindari membuat service saya ikut tidak stabil. Biasanya saya pasang timeout eksplisit, retry terbatas dengan backoff, dan fallback behavior yang aman terhadap data. Saya juga bedakan endpoint mana yang boleh degrade dan mana yang harus fail-fast supaya user expectation tetap jelas. Kalau dependency terus tidak sehat, saya dorong mitigasi jangka menengah seperti queue buffering atau circuit breaker. Intinya, saya desain layanan agar dependency issue tidak langsung jadi outage internal.

Struktur cepat:
- Konteks singkat: dependency timeout bisa menular jadi gangguan sistem internal.
- Aksi utama: terapkan timeout, backoff retry, dan fallback sesuai criticality.
- Keputusan teknis/non-teknis: lindungi stabilitas layanan inti meski dependency bermasalah.
- Hasil terukur: timeout cascade berkurang dan pengalaman user lebih prediktif.
- Closing relevan ke posisi: saya terbiasa membangun ketahanan terhadap dependency eksternal.

## 3) Versi Ultra Singkat (10-20 detik)

> Dependency timeout saya tangani dengan timeout+retry terkontrol+fallback aman, supaya layanan utama tetap stabil.
...
- Pertanyaan: Bagaimana kamu menangani dependency eksternal yang sering timeout?
