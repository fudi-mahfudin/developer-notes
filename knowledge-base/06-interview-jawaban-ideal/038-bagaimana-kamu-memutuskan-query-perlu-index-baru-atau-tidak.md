# Bagaimana kamu memutuskan query perlu index baru atau tidak?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana kamu memutuskan query perlu index baru atau tidak?
...
> Saya tidak menambah index berdasarkan feeling; saya lihat dulu query plan, cardinality, dan pola akses real di production. Kalau query lambat karena full scan pada jalur yang sering dipakai, baru saya evaluasi index yang paling sesuai. Saya juga hitung trade-off write overhead karena index tambahan bisa memperlambat insert/update. Di praktiknya, saya treat index sebagai keputusan biaya-manfaat, bukan default fix semua query lambat. Dengan cara ini, performa naik tanpa membuat beban write memburuk diam-diam.

Struktur cepat:
- Konteks singkat: index yang salah bisa memperbaiki read tapi merusak write.
- Aksi utama: analisis query plan dan frekuensi akses sebelum menambah index.
- Keputusan teknis/non-teknis: optimasi dilakukan berbasis data, bukan asumsi.
- Hasil terukur: query kritikal membaik tanpa menimbulkan bottleneck baru.
- Closing relevan ke posisi: saya terbiasa balancing performa read/write di lingkungan operasional.

## 3) Versi Ultra Singkat (10-20 detik)

> Index saya tambah berdasarkan evidence query plan dan pola traffic, sambil mempertimbangkan biaya di sisi write.
...
- Pertanyaan: Bagaimana kamu memutuskan query perlu index baru atau tidak?
