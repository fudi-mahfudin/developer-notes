# Apa pendekatan kamu untuk error handling API di production?

## 1) Pertanyaan

- Pertanyaan interviewer: Apa pendekatan kamu untuk error handling API di production?
...
> Untuk error handling API di production, saya bedakan jelas antara error bisnis, error validasi, dan error sistem. Tujuannya supaya respons ke klien konsisten dan tim internal bisa investigasi cepat tanpa menerka-nerka. Saya juga pastikan setiap error punya correlation context agar trace lintas service tetap utuh. Di pengalaman saya, struktur error yang rapi jauh mengurangi waktu debugging saat traffic tinggi. Jadi pendekatan saya bukan sekadar `try/catch`, tapi desain error contract yang operasional.

Struktur cepat:
- Konteks singkat: error acak bikin klien bingung dan debugging lambat.
- Aksi utama: klasifikasikan error dan standarkan format respons.
- Keputusan teknis/non-teknis: sertakan context untuk observability lintas service.
- Hasil terukur: waktu investigasi issue production lebih singkat.
- Closing relevan ke posisi: API jadi lebih stabil untuk user dan engineer.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya desain error handling berdasarkan jenis error + contract konsisten + trace context, supaya klien jelas dan tim cepat debug.
...
- Pertanyaan: Apa pendekatan kamu untuk error handling API di production?
