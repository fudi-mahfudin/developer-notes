# Bagaimana strategi validasi request body agar konsisten lintas service?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana strategi validasi request body agar konsisten lintas service?
...
> Untuk validasi request body lintas service, saya biasakan kontrak skema yang konsisten di satu sumber kebenaran, bukan validasi ad-hoc per endpoint. Saya biasanya pakai schema validation di boundary API, lalu error mapping dibuat seragam supaya tim klien tidak bingung. Saat requirement berubah, skema saya versioned agar transisi tidak merusak klien lama. Ini penting karena saya sering kerja di integrasi lintas sistem yang datanya tidak selalu bersih. Dengan pendekatan ini, bug karena payload ambigu bisa ditekan dari awal.

Struktur cepat:
- Konteks singkat: validasi yang tidak konsisten memicu bug integrasi berulang.
- Aksi utama: gunakan schema validation terpusat di API boundary.
- Keputusan teknis/non-teknis: kontrak error diseragamkan untuk semua klien.
- Hasil terukur: kegagalan karena payload invalid turun dan debugging lebih cepat.
- Closing relevan ke posisi: ini fondasi API yang scalable dan maintainable.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya standarkan validasi lewat skema terpusat + error format konsisten, supaya lintas service tidak saling menebak kontrak.
...
- Pertanyaan: Bagaimana strategi validasi request body agar konsisten lintas service?
