# Bagaimana kamu memastikan API response tetap stabil saat refactor backend?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana kamu memastikan API response tetap stabil saat refactor backend?
...
> Saat refactor backend, saya menjaga response API tetap stabil dengan dua lapis kontrol: contract test otomatis dan compatibility checklist sebelum merge. Saya juga pisahkan refactor internal dari perubahan kontrak eksternal supaya klien tidak ikut terdampak. Kalau memang perlu ubah shape data, saya jalankan mode kompatibilitas sementara selama masa migrasi. Pola ini sering saya pakai agar tim bisa meningkatkan kualitas kode tanpa merusak konsumsi API existing. Intinya, refactor boleh agresif di dalam, tapi harus konservatif di kontrak publik.

Struktur cepat:
- Konteks singkat: refactor rawan mengubah perilaku API tanpa sengaja.
- Aksi utama: enforce contract test dan review kompatibilitas sebelum rilis.
- Keputusan teknis/non-teknis: internal boleh berubah, kontrak publik harus stabil.
- Hasil terukur: risiko regressions di klien turun signifikan.
- Closing relevan ke posisi: tim tetap bisa improve codebase dengan aman.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya jaga stabilitas response lewat contract test + compatibility checklist, jadi refactor internal tidak jadi breaking change eksternal.
...
- Pertanyaan: Bagaimana kamu memastikan API response tetap stabil saat refactor backend?
