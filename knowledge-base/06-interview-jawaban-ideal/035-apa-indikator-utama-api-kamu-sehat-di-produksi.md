# Apa indikator utama API kamu sehat di produksi?

## 1) Pertanyaan

- Pertanyaan interviewer: Apa indikator utama API kamu sehat di produksi?
...
> Indikator utama API sehat buat saya bukan satu metrik, tapi kombinasi: success rate, latency p95/p99, dan error budget trend. Saya tambah satu lapis lagi: dependency health, karena API bisa terlihat sehat padahal downstream bermasalah. Di Siloam, saya belajar bahwa metrik harus dibaca per endpoint kritikal, bukan agregat global yang menipu. Saya juga lihat pola retry dan timeout untuk mendeteksi masalah sebelum user mengeluh. Jadi “sehat” menurut saya adalah layanan stabil, prediktif, dan masih punya ruang aman terhadap lonjakan beban.

Struktur cepat:
- Konteks singkat: metrik tunggal sering menutupi gejala masalah sebenarnya.
- Aksi utama: pantau health per endpoint kritikal + dependency terkait.
- Keputusan teknis/non-teknis: fokus pada indikator yang actionable untuk operasi harian.
- Hasil terukur: anomali bisa dideteksi lebih awal sebelum jadi insiden besar.
- Closing relevan ke posisi: saya terbiasa membaca kesehatan API sebagai sistem, bukan endpoint terisolasi.

## 3) Versi Ultra Singkat (10-20 detik)

> API sehat itu kombinasi success rate, latency, error budget, dan kesehatan dependency—bukan angka tunggal.
...
- Pertanyaan: Apa indikator utama API kamu sehat di produksi?
