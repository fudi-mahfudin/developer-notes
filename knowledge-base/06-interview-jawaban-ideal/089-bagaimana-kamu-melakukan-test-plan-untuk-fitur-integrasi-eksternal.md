# Bagaimana kamu melakukan test plan untuk fitur integrasi eksternal?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana kamu melakukan test plan untuk fitur integrasi eksternal?
...
> Test plan untuk integrasi eksternal saya susun berbasis failure scenario, bukan hanya daftar endpoint yang dipanggil. Saya bagi skenario menjadi normal flow, partial failure, timeout, duplicate event, dan recovery path. Setiap skenario saya kaitkan dengan expected behavior dan indikator yang harus dipantau saat uji. Saya juga libatkan tim operasional untuk memastikan test plan mencerminkan kondisi lapangan, bukan hanya asumsi developer. Hasilnya test plan lebih realistis dan benar-benar mempersiapkan tim untuk kondisi production.

Struktur cepat:
- Konteks singkat: integrasi eksternal paling sering gagal di jalur non-happy path.
- Aksi utama: rancang test berdasarkan mode kegagalan utama dan recovery yang diharapkan.
- Keputusan teknis/non-teknis: validasi skenario bersama tim operasional agar relevan lapangan.
- Hasil terukur: kesiapan tim menghadapi gangguan integrasi meningkat.
- Closing relevan ke posisi: saya menilai integrasi dari ketahanan, bukan sekadar koneksi berhasil.

## 3) Versi Ultra Singkat (10-20 detik)

> Test plan integrasi saya berpusat pada failure scenario dan recovery behavior, supaya siap menghadapi kondisi production nyata.
...
- Pertanyaan: Bagaimana kamu melakukan test plan untuk fitur integrasi eksternal?
