# Cara menangani retry tanpa bikin duplicate side effect?

## 1) Pertanyaan

- Pertanyaan interviewer: Cara menangani retry tanpa bikin duplicate side effect?
- Kategori: General
...
> Retry tanpa duplicate side effect saya atur lewat kombinasi retry budget, idempotency key, dan klasifikasi error retryable vs non-retryable. Strategi saya safe-retry envelope: retry hanya untuk kegagalan sementara dan selalu dalam batas yang terukur. Dengan ini, reliability naik tanpa membuat data jadi duplikat.

Struktur cepat:
- Konteks singkat: pada kasus "Cara menangani retry tanpa bikin duplicate side effect?", saya identifikasi risiko operasional paling kritikal lebih dulu.
...
> Saya pakai safe-retry envelope: retry terkontrol + idempotency agar aman dari duplicate side effect.
...
- Definisi strategi: safe-retry envelope = kerangka retry yang aman lewat batas percobaan, klasifikasi error, dan idempotensi.
...
- Pertanyaan: Cara menangani retry tanpa bikin duplicate side effect?
