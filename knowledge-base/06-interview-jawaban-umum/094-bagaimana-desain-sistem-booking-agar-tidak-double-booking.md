# Bagaimana desain sistem booking agar tidak double booking?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana desain sistem booking agar tidak double booking?
...
> Sistem booking anti double-booking saya bangun dengan locking/transaction boundary yang jelas pada resource dan timeslot. Strategi saya consistency-guarded reservation: alokasi slot harus atomik agar dua request bersamaan tidak sama-sama sukses. Di layer API, saya tambah idempotency untuk menahan duplikasi dari retry client.

Struktur cepat:
- Konteks singkat: pada kasus "Bagaimana desain sistem booking agar tidak double booking?", saya mulai dari risiko tertinggi yang memengaruhi user dan operasi.
...
> Saya cegah double booking lewat reservasi atomik + idempotensi di API agar concurrency tetap aman.
...
- Definisi strategi: consistency-guarded reservation = mekanisme reservasi atomik yang melindungi konsistensi saat concurrent request.
...
- Pertanyaan: Bagaimana desain sistem booking agar tidak double booking?
