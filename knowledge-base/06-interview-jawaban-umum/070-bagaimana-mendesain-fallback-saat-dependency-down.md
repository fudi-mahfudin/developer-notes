# Bagaimana mendesain fallback saat dependency down?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana mendesain fallback saat dependency down?
...
> Fallback saat dependency down saya desain bertingkat: static fallback, cached response, atau degrade mode sesuai kritikalitas fitur. Strategi saya graceful-degradation ladder: turunkan level layanan secara terkontrol, bukan langsung full outage. Dengan begitu, user masih mendapat fungsi minimum sambil tim memulihkan akar masalah.

Struktur cepat:
- Konteks singkat: pada kasus "Bagaimana mendesain fallback saat dependency down?", saya identifikasi risiko operasional paling kritikal lebih dulu.
...
> Saya bangun fallback bertingkat agar saat dependency down layanan tetap ada mode minimum yang aman.
...
- Definisi strategi: graceful-degradation ladder = urutan fallback bertingkat untuk menjaga layanan tetap tersedia saat gangguan.
...
- Pertanyaan: Bagaimana mendesain fallback saat dependency down?
