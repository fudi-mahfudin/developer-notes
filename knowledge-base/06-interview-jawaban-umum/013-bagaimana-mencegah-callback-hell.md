# Bagaimana mencegah callback hell?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana mencegah callback hell?
...
> Untuk mencegah callback hell, saya mulai dari desain alur async yang jelas: pisahkan orkestrasi proses dari implementasi step per step. Di codebase Node.js, saya biasakan `async/await` dengan function kecil yang punya tanggung jawab tunggal, lalu error handling dipusatkan agar tidak tersebar. Strategi saya adalah layered async flow, yaitu membuat struktur bertingkat yang mudah dibaca dan diuji. Kalau ada operasi independen, saya jalankan paralel terkontrol supaya tidak menambah nesting yang tidak perlu. Hasilnya kode lebih maintainable dan onboarding tim lebih cepat.

Struktur cepat:
- Konteks singkat: callback hell menurunkan readability dan memperbesar bug handling.
- Aksi utama: refactor ke async/await dengan pemisahan tanggung jawab fungsi.
- Keputusan teknis/non-teknis: sentralisasi error path agar perilaku konsisten.
- Hasil terukur: kode lebih mudah review, test, dan dikembangkan.
- Closing relevan ke posisi: saya menjaga kualitas async flow untuk sustain delivery tim.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya cegah callback hell dengan strategi layered async flow: orkestrasi jelas, fungsi kecil, dan error handling terpusat.
...
- Definisi strategi: layered async flow = susun proses asinkron per lapisan tanggung jawab agar alur tetap linear, teruji, dan mudah dirawat.
...
- Pertanyaan: Bagaimana mencegah callback hell?
