# Bagaimana membuat worker job idempotent?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana membuat worker job idempotent?
...
> Worker job idempotent saya bangun dengan idempotency key, state transition yang jelas, dan dedup store untuk hasil eksekusi. Strategi saya idempotent worker contract: setiap job boleh diproses ulang tanpa menghasilkan side effect ganda. Ini penting saat retry otomatis berjalan di bawah load atau gangguan jaringan.

Struktur cepat:
- Konteks singkat: pada kasus "Bagaimana membuat worker job idempotent?", saya identifikasi risiko operasional paling kritikal lebih dulu.
...
> Worker idempotent berarti retry aman; saya jaga lewat idempotency key dan state transition yang ketat.
...
- Definisi strategi: idempotent worker contract = kontrak eksekusi job yang menjamin hasil bisnis tetap satu meski job diproses ulang.
...
- Pertanyaan: Bagaimana membuat worker job idempotent?
