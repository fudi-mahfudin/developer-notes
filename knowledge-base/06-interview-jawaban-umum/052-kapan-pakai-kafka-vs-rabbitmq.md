# Kapan pakai Kafka vs RabbitMQ?

## 1) Pertanyaan

- Pertanyaan interviewer: Kapan pakai Kafka vs RabbitMQ?
...
> Kafka saya pilih saat butuh throughput tinggi, replay event, dan stream processing; RabbitMQ saya pilih untuk routing fleksibel dan workload task queue yang lebih klasik. Strategi saya workload-fit broker selection: broker dipilih dari pola pesan dan reliability requirement, bukan preferensi tim. Dengan itu, biaya operasional dan kompleksitas bisa tetap proporsional.

Struktur cepat:
- Konteks singkat: pada kasus "Kapan pakai Kafka vs RabbitMQ?", saya identifikasi risiko operasional paling kritikal lebih dulu.
...
> Kafka untuk event stream skala besar, RabbitMQ untuk queue/routing fleksibel; saya pilih berdasarkan karakter workload.
...
- Definisi strategi: workload-fit broker selection = memilih message broker dari kebutuhan throughput, ordering, dan pola konsumsi event.
...
- Pertanyaan: Kapan pakai Kafka vs RabbitMQ?
