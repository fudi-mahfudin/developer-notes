# Metrik minimum apa yang wajib dipantau untuk API service?

## 1) Pertanyaan

- Pertanyaan interviewer: Metrik minimum apa yang wajib dipantau untuk API service?
...
> Metrik minimum API service yang saya wajibkan: latency (p95/p99), error rate, throughput, saturation/resource, plus dependency health. Strategi saya SLO-aligned metric set: metrik dipilih berdasarkan target reliabilitas, bukan sekadar banyaknya dashboard. Dengan set minimum yang tepat, alert jadi relevan dan keputusan kapasitas lebih akurat.

Struktur cepat:
- Konteks singkat: pada kasus "Metrik minimum apa yang wajib dipantau untuk API service?", saya identifikasi risiko operasional paling kritikal lebih dulu.
...
> Saya pantau latency, error, throughput, saturation, dan dependency health sebagai set minimum yang selaras SLO.
...
- Definisi strategi: SLO-aligned metric set = pemilihan metrik inti yang langsung mencerminkan target reliabilitas layanan.
...
- Pertanyaan: Metrik minimum apa yang wajib dipantau untuk API service?
