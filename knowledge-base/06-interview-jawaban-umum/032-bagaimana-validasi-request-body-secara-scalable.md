# Bagaimana validasi request body secara scalable?

## 1) Pertanyaan

- Pertanyaan interviewer: Bagaimana validasi request body secara scalable?
- Kategori: General
...
> Validasi request body yang scalable saya bangun lewat schema layer terpusat per domain, jadi rule tidak tercecer di controller. Strategi saya schema-as-source-of-truth: kontrak request dipakai bersama oleh handler, test, dan dokumentasi. Saya tambahkan versi schema saat ada perubahan besar supaya backward compatibility tetap aman. Hasilnya, error validasi lebih konsisten dan onboarding developer baru lebih cepat.

Struktur cepat:
- Konteks singkat: validasi manual tersebar cepat jadi technical debt.
- Aksi utama: pusatkan rule validasi dalam schema versioned per domain.
- Keputusan teknis/non-teknis: samakan format error validasi lintas endpoint.
- Hasil terukur: bug input invalid di layer bisnis menurun.
- Closing relevan ke posisi: saya membangun fondasi API yang mudah diskalakan tim.

## 3) Versi Ultra Singkat (10-20 detik)

> Saya pakai schema-as-source-of-truth agar validasi body konsisten, reusable, dan aman saat sistem berkembang.
...
- Definisi strategi: schema-as-source-of-truth = schema menjadi acuan tunggal untuk validasi, dokumentasi, dan pengujian input.
...
- Pertanyaan: Bagaimana validasi request body secara scalable?
