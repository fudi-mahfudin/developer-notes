# Bedanya unit test, integration test, dan e2e test?

## 1) Pertanyaan

- Pertanyaan interviewer: Bedanya unit test, integration test, dan e2e test?
...
> Unit test fokus ke logika kecil terisolasi, integration test memverifikasi interaksi antarmodul/dependency, dan e2e memvalidasi alur user dari ujung ke ujung. Strategi saya test-purpose clarity: sebelum menulis test, saya tentukan dulu pertanyaan apa yang ingin dijawab di level tersebut. Ini mencegah test overlap yang membuat pipeline lambat tapi tidak menambah confidence.

Struktur cepat:
- Konteks singkat: pada kasus "Bedanya unit test, integration test, dan e2e test?", saya mulai dari risiko tertinggi yang memengaruhi user dan operasi.
...
> Saya bedakan jenis test dari pertanyaan yang diuji: unit untuk logika, integration untuk interaksi, e2e untuk alur bisnis.
...
- Definisi strategi: test-purpose clarity = memilih jenis test berdasarkan tujuan validasi paling tepat.
...
- Pertanyaan: Bedanya unit test, integration test, dan e2e test?
