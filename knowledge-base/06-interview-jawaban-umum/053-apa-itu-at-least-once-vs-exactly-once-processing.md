# Apa itu at-least-once vs exactly-once processing?

## 1) Pertanyaan

- Pertanyaan interviewer: Apa itu at-least-once vs exactly-once processing?
...
> At-least-once berarti pesan bisa terkirim ulang sehingga consumer wajib idempotent; exactly-once lebih ketat tapi biaya implementasinya lebih tinggi. Strategi saya semantics-aware processing: pilih delivery guarantee berdasarkan dampak bisnis jika terjadi duplikasi. Untuk sistem healthcare, saya biasanya kombinasikan at-least-once dengan idempotency key agar aman sekaligus pragmatis.

Struktur cepat:
- Konteks singkat: pada kasus "Apa itu at-least-once vs exactly-once processing?", saya identifikasi risiko operasional paling kritikal lebih dulu.
...
> Saya pilih jaminan delivery berdasarkan risiko bisnis, lalu lindungi duplikasi dengan idempotency di consumer.
...
- Definisi strategi: semantics-aware processing = menyesuaikan model delivery message dengan toleransi risiko bisnis dan biaya implementasi.
...
- Pertanyaan: Apa itu at-least-once vs exactly-once processing?
