# Kapan pakai index, dan apa trade-off-nya?

## 1) Pertanyaan

- Pertanyaan interviewer: Kapan pakai index, dan apa trade-off-nya?
...
> Saya menambah index saat query kritikal sering melakukan full scan dan sudah terbukti jadi bottleneck di metrik produksi. Strategi saya measure-before-index: keputusan indexing harus didahului profil query, bukan asumsi. Trade-off yang saya jelaskan selalu jelas: read jadi lebih cepat, tapi write cost dan storage naik. Karena itu saya prioritaskan index untuk path yang read-heavy dan bernilai bisnis tinggi.

Struktur cepat:
- Konteks singkat: index tidak gratis, bisa memperlambat write jika berlebihan.
- Aksi utama: analisis execution plan dan frekuensi query sebelum indexing.
- Keputusan teknis/non-teknis: fokus index pada query read-heavy bernilai tinggi.
- Hasil terukur: latency query kritikal turun tanpa over-indexing.
- Closing relevan ke posisi: saya mengoptimasi database secara data-driven.

## 3) Versi Ultra Singkat (10-20 detik)

> Index saya tambah berbasis bukti bottleneck; saya sadar trade-off-nya adalah write lebih mahal dan storage bertambah.
...
- Definisi strategi: measure-before-index = indexing dilakukan setelah pembuktian bottleneck dan evaluasi trade-off read/write.
...
- Pertanyaan: Kapan pakai index, dan apa trade-off-nya?
